...
//
     myTree[iLast].left->parent   =  myTree + iLast;  // curent node is
     myTree[iLast].right->parent  =  myTree + iLast;  // parent of new nodes
//
     }
...
//
// foward transversing of the tree (example)
//
 printf("\n");
 ...
 for (int iDepth = 0; iDepth < depth; iDepth++)
   ...
   printf("# %5d |%5d %17ld\n", iDepth, currentNode->myData, 
		               (long int)currentNode);
   currentNode = (iDepth%2==0) ? currentNode->left
	                       : currentNode->right;
   }
//
// back transversing of the tree (example)
//
 printf("\n");
 currentNode = allNodes + nNodes - 1;  // start with last
 while (currentNode!=NULL)
   {
   printf("# backward %5d %17ld\n", currentNode->myData, 
		               (long int)currentNode);
